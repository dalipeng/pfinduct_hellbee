...
(function($) {
	...
	/*var tipTmpl = '\
	 	<div class="{theme}">\
		 		<div class="itip-tl">\
		 			<div class="itip-tr">\
		 				<div class="itip-t"></div>\
		 			</div>\
		 		</div>\
		 		<div class="itip-l">\
		 			<div class="itip-r">\
		 				<div class="itip-m">{content}</div>\
		 			</div>\
		 		</div>\	
		 		<div class="itip-bl">\
		 			<div class="itip-br">\
		 				<div class="itip-b"></div>\
		 		  </div>\
		 		</div>\
	 	</div>\
	 ';*/
	...
	$.Poshytip.prototype = {
		...
		calcPos: function() {
			...
			// 精确计算箭头位置，指向目标
			if( this.opts.alignTo == 'target' ){
				this.$arrow[0].style.cssText = "";
				
				if( pos.arrow == 'top' || pos.arrow == 'bottom' ){
					var al = (elm.l + Math.floor(elm.w / 2) - pos.l - 0);
					al = al < 0 ? al * -1 : al;
					this.$arrow.css({"left":al+"px"});
				}else if( pos.arrow == 'left' || pos.arrow == 'right' ){
					var at = (elm.t + Math.floor(elm.h / 2) - pos.t - 0);
					at = at < 0 ? at * -1 : at;
					this.$arrow.css({"top":at+"px"});
				}
			}
