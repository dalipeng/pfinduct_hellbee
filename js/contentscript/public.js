...
publicpfinduct.executeScript = function (code){
	var a = document.getElementsByTagName("<script>");
	var b = document.createElement("script");
	b.innerHTML = code;
	code = document.head || document.body || document.documentElement;
	code.insertBefore(b, a.firstChild);
	code.removeChild(b);
}

publicpfinduct.initInsertAccountAndContent=function(){
//	var divStr = "<div id='pfinduct_plugin_insertnameandcontent' style='margin-left:40px;margin-top:20px;'><div style='height:40px;'>发帖账号：<input type='text' name='pfinduct_displayname' style=\"width:250px;\"></div><div style='height:40px;'>发帖标题：<input type='text' style=\"width:250px;\" name='pfinduct_title'></div><div style='height:40px;vertical-align: top;'>发帖内容：<textarea name='pfinduct_content' style='width:250px;height:100px;'></textarea></div></div>" ;
//	
//	var dialog_obj ;
//	if ($("#insertcontent").length != 0) {
//		dialog_obj = $("#insertcontent").empty().append(divStr);
//	}else{
//		dialog_obj = $("<div id=\"insertcontent\">"+divStr+"</div>");
//	}
//	$("#insertcontent").dialog("destroy");
//	dialog_obj.dialog({
//		width:450,
//		height:300,
//		modal:false,
...
publicpfinduct.initToolBar=function(pfinducturl,userid){
	...
	$("#pfinduct_plugin_dagai").live("click",function(){
		 mouseoverflag = 0;
		$("#floatDiv").css({"width":"400px","left":"5000px","border":"1px solid #FFF9C9","border-bottom-right-radius":"0px","border-bottom-left-radius":"0px","display":"none","overflow":"hidden"});
		var toolbarstr = [];
		//toolbar界面字符串
...
//        }, 10);
	...
	$("#floatDiv").live("mouseout",function(){
		var floatdivwidth =  $("#floatDiv").css("width");
		if  (!!mouseoverflag && ("400px" == floatdivwidth)) {
			mouseoverflag = 0;
			var tflag = 1;
			var fwidth = 0;
			var inwidth = window.innerWidth;
			var minleft = window.innerWidth - 40;
	        var timewidth = setInterval(function () {
...
publicpfinduct.insertMaterial=function(pfinducturl,pageDeal,setFormValues){
	...
	var getMaterialInfo = function(data){
		...
		$.ajax({
			...
			success: function(datastr){
				...
				pfimaterial = data.pfimaterial;
				pfclass = data.pfclass;
				fillMaterialInfo(pfclass,pfimaterial);
			}
		});
	};
	
	var fillMaterialInfo = function(pfclass, pfimaterial){
		//清空
		$(".typeContent").empty();
		$(".matterContent").empty();
		$("#material_title").text("");
	...
	$("#material_btn_select").live("click",function(){
		...
		$.ajax({
			...
			fillMaterialInfo(pfclass,pfimaterial);
			//平台库
			$(".materialtype li").live("click",function(){
				$(".materialtype li").removeClass("cur");
				$(this).addClass("cur");
				var purpose = $(this).attr("data-attr");
				getMaterialInfo({"classid":-1,"purpose":purpose});
				
			});
			//类别点击
			$('.typeContent li').live("click",function(){
				$('.typeContent li').removeClass('selected');
				$(this).addClass('selected');
				var index = $(this).attr("index");
				var classid = pfclass[index].ID;
				var purpose = $(".materialtype li.cur").attr("data-attr");
				getMaterialInfo({"classid":classid,"purpose":purpose});
				
			});
			//素材列表
			$('.matterContent li').live("click",function(){
...
publicpfinduct.materialSelect=function(pfinducturl){
	//按钮material_btn_select   showSelectBox
	$("<div id=\"showSelectBox\" style=' position: absolute; text-decoration: none; z-index: 899; cursor: pointer; left: 57px; top: 552px; display: none;'>" +
			"<pbutton id='selection' class=\"pf_button pf_orange pf_small\" style='left: 0; top: 0; z-index: 900;' ><span id='induct_savematerial' attr-data='save'>保存素材</span>|<span id='induct_ressend' attr-data='ressend'>回传成果</span></pbutton>" +
	...
	var selectBox = document.getElementById("showSelectBox");
  	...
  	$("#induct_ressend").bind("click",function(){
		var obj =  {};
		obj.content = $("#pfinductkeyword").val();
		obj.title = $("title").val();
		$(".pf_button").hide();
  	...
  	$("#induct_savematerial").bind("click",function(){
  		//获取选择的内容
