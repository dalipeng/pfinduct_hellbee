...
var pageinit = function(settings){
	...
	var regArr=[
		/http:\/\/www\.haianw\.com.*/,
		/http:\/\/www\.haianw\.com\/((forum.php\?mod=post&action=newthread.+)|(forum-.+))/,
		/http:\/\/www\.haianw\.com\/((forum.php\?mod=viewthread.+)|(thread.+)|(forum\.php\?mod=post&action=reply.+))/,
		/http:\/\/www\.haianw\.com\/forum-.+/
	];
	state = publicpfinduct.URLState(regArr,currenturl);
	console.log("haianw state:"+state);
	...
	if(state>0){
		pageInfoData = haianw.getPageInfo();
		//检查界面登录状态
		publicpfinduct.checkPageInfo(pageInfoData);
	}
	//素材注入
	if(state==1||state==2){
		publicpfinduct.insertMaterial(pfinducturl,haianw.pageDeal,haianw.setFormValues);
	}
	
	//回传
	if(state==1||state==2){
		publicpfinduct.sendBack(pfinducturl,userid,pageInfoData,haianw.hc_button,haianw.listValue);
	}
	...
	//账号选择
	if(state>=0){
		haianw.fillAccountAndPwd(pfinducturl,currenturl,userid);
	}
 ...
 * 发帖区域赋值
 */
haianw.setFormValues=function(title,content){
	if(state==1){
		if($("#fastpostform").length>0){
			//原创
			$("#typeid_fast").val(60);
...
/**取界面元素  **/
haianw.getPageInfo=function(){
	var userObj = $("#toptb strong>a");
	var displayname = userObj.text();
	var accountname = "";
	var formnameobj = $("#pt>.z>a:eq(3)");
	var forumname = "海安网论坛-"+formnameobj.text();
 ...
 * 回传按钮注入
 */
haianw.hc_button=function(page){
	...
	if(state==1||state==3){
		...
		if($("#postlist").length>0){
			$("#postlist>div[id^=post]").each(function(){
				var username = $(this).find("td.pls>div.pls>div.pi>div.authi a").text();
				username = $.trim(username);
				console.log(username+"~~~"+displayname);
				if(username==displayname){
					$(this).find("td.plc div.authi").append("<pbutton  class='pf_button pf_orange pf_small' name='btn_up'>回传</pbutton>");
 ...
 * 选择填充账号
 */
haianw.fillAccountAndPwd=function(pfinducturl,currenturl,userid){
	$("#ls_username").unbind("click").die("click").live("click",function(){
		...
		publicpfinduct.getCurrentTaskid(pfinducturl,function(taskid){
			account.selectAccount(function(username,password){
				...
				$("button[type='submit'].comiis_dlan").click();
